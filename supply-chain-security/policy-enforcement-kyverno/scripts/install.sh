#!/bin/bash
# Background script: installs Tekton Pipelines, Tekton Chains, tkn CLI, cosign, Kyverno, and a local registry
...
# Install Kyverno v1.12.6
kubectl apply -f https://github.com/kyverno/kyverno/releases/download/v1.12.6/install.yaml

# Wait for Kyverno pods to be ready (may take a minute)
kubectl wait --for=condition=ready pod -l app.kubernetes.io/part-of=kyverno \
  -n kyverno --timeout=180s

echo "Tekton Pipelines, Chains, Kyverno, tkn, cosign, and local registry are ready!"
